...
cr = 0.0045; %rolling resistance (safety factor to 0.004)
g = 9.81; %m/s^2
mass_car = 45; %kg 
mass_driver = 50; %kg
...
%Efficiency and dynamic calculations
    ...
    eff_m(i) = motor_coeff(1)*Tm(i)^6 + motor_coeff(2)*Tm(i)^5 + motor_coeff(3)*Tm(i)^4+ motor_coeff(4)*Tm(i)^3 + ...
...
for i = 1:1:total_points
    if P(i) < 99
    %     eff_h(i) = eff_h_coeff_a(1)*P(i)^5 + eff_h_coeff_a(2)*P(i)^4 + eff_h_coeff_a(3)*P(i)^3 ...
    %         + eff_h_coeff_a(4)*P(i)^2 + eff_h_coeff_a(5)*P(i) + eff_h_coeff_a(6);
        eff_h(i) = 0.40;
    else
...
nominal_flow = 0.01; % in l/min
nominal_P_H = (nominal_flow / (1000 * 60)) * p_H * NCV_H * 1000;
...
for i = 1:1:total_points
    P_FC(i) = P(i) / (eff_m(i)*0.01 * eff_h(i) * eff_c);
end
...
for i = 1:1:((total_points)-1) %t and E_H consumed between points (i and i+1)
    ...
    if P(i) == 0
        ...
        E_H(i) = P(i)*t(i)/(eff_h(i)*eff_c*(eff_m(i)*0.01) );
    end
    
end

for i = 1:1:total_points
    P_H(i) = P(i)/ (eff_h(i)*(eff_m(i)*0.01) );
    if P_H(i) == 0
        P_H(i) =  nominal_P_H;
    end
end    
...
Fuel_eff = (NCV_H*p_H*total_length*0.001)/(E_H_total*0.001); %km/m^3
Fuel_cons = (total_length * 0.001) / (Fuel_eff * 0.001); %l

total_fuel = 80; %l
Fuel_cons_perc = Fuel_cons / total_fuel * 100; % percent

p_G = 764.6; %kg/m^3
NCV_G = 42900; %kJ/kg 
Fuel_eff_gasoline = Fuel_eff*NCV_G*p_G/(p_H*NCV_H*1000); %km/l
...
disp(['Total fuel consumption: ',num2str(Fuel_cons_perc),' %']);
