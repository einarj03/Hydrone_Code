...
coeff = [-1.89559261 25.04722276 -129.6267401 333.2213722 -447.3435524 ...
...
%Efficiency and dynamic calculations
    ...
    eff_m(i) = coeff(1)*Tm(i)^6 + coeff(2)*Tm(i)^5 + coeff(3)*Tm(i)^4+ coeff(4)*Tm(i)^3 + ...
               coeff(5)*Tm(i)^2 + coeff(6)*Tm(i) + coeff(7);
...
nominal_flow = 0; % in l/min
% nominal_P = (nominal_flow / (1000 * 60)) * p_H * NCV_H;

%add dynamic eff calcs in here  - dependent upon speed.

for i = 1:1:((total_points)-1) %t and E_H consumed between points (i and i+1)
    t(i) = 2*s/(v(i+1) + v(i));
    E_H(i) = P(i)*t(i)/( eff_h*eff_c*(eff_m(i)*0.01) );
%     if E_H(i) == 0
%         E_H(i) =  nominal_P * t(i);
%     end
    
% % %     Add hydrogen energy consumption at 0 Power demad, flow rate is
% % %     0.108 l/min
end

for i = 1:1:total_points
    P_H(i) = P(i)/ (eff_h*(eff_m(i)*0.01) );
end    
