...
class RWThread(Thread):
    ...
    def run(self):
        ...
        while not self.cancelled:
            ...
            posI1 = DM.DataManager.getIdealPosID()
            
            sendAr = [LongLat[0], LongLat[1], DM.DataManager.getGPSSpeed(), DM.DataManager.getFCons(), simData[posI].VSpeedIn]
...
class SimThread(Thread):
    ...
    def calcPlan(self):
        ...
        curPos1 = DM.DataManager.getPosID1()
        for i in range(curPos, (curPos + TD.dataLength)):
            RL.i = i
            RL.Position = TD.Distance[i]
            RL = Sim.Throttle(TD, RL, CD, RD)
